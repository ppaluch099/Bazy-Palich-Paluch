; (function ($, window, document, undefined) {
  ...
  Plugin.prototype = {
    ...
    check: function (value) {
      ...
      this.addStatus(securePercentage, value.length);

    },

    addStatus: function (percentage, counter) {
      var status = "",
      ...
      meter.attr("class", "passtrengthMeter");
        if (percentage >= 25 && counter >=4) {
          meter.attr("class", "passtrengthMeter");
          status = "weak";
          text = this.options.textWeak;
        }
        if (percentage >= 50 && counter >=6) {
          meter.attr("class", "passtrengthMeter");
          status = "medium";
          text = this.options.textMedium;
        }
        if (percentage >= 75 && counter >=8) {
          meter.attr("class", "passtrengthMeter");
          status = "strong";
          text = this.options.textStrong;
        }
        if (percentage >= 100 && counter >=10) {
          meter.attr("class", "passtrengthMeter");
          status = "very-strong";
